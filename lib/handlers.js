              ...
              lib.delete("users", email, function (err) {
                if (!err) {
                  ...
                  checks.forEach((check) => {
                    lib.delete("checks", check, function (err) {
                      if (err) {
                        deletingErrors = true;
                      }
                      noOfChecks += 1;
                      if (noOfChecks === checks.length) {
                        if (!deletingErrors) {
                          callback(200);
                        } else {
                          callback(500, {
                            Error: "Could not delete all checks of the user.",
                          });
                        }
                      }
                    });
                  ...
                  callback(404, { Error: "Could not delete user." });
                }
              });
            } else {
...
handlers._checks = {
  get: function (data, callback) {
    ...
    const id =
      typeof query.id === "string" && query.id.length === 20 ? query.id : "";
    ...
    if (id) {
      lib.read("checks", id, function (err, checkData) {
        if (!err && checkData) {
          ...
          callback(400, { Error: "The requested id does not exist" });
        }
      });
    } else {
  ...
  post: function (data, callback) {
    ...
    if (protocol && url && method && successCodes && timeoutSeconds) {
      // Lookup the user by the valid token
      const token =
        typeof data.headers.token === "string" ? data.headers.token : "";
      if (token) {
              ...
              if (!err && userData) {
                const userChecks =
                  ...
                  userData.checks instanceof Array
                    ? userData.checks
                    : [];
                if (userChecks.length < mode.maxChecks) {
  ...
  put: function (data, callback) {
    ...
    if (id) {
      lib.read("checks", id, function (err, checkData) {
        if (!err && checkData) {
          const userEmail =
            typeof checkData.userEmail === "string" ? checkData.userEmail : "";

          // Check the validatity of the token
          handlers._tokens.verifyToken(token, userEmail, function (isVerified) {
            if (isVerified) {
              const updatedCheckData = {
                ...checkData,
                protocol: protocol ? protocol : checkData.protocol,
                userEmail: userEmail ? userEmail : checkData.userEmail,
              ...
              lib.update("checks", id, updatedCheckData, function (err) {
                if (!err) {
                  callback(200);
                } else {
                  callback(500, {
                    Error: "Updating checkdata was not successfull.",
                  });
                }
              });
            } else {
            ...
            if (isVerified) {
              lib.delete("checks", id, function (err) {
                if (!err) {
                  lib.read("users", userEmail, function (err, userData) {
                    if (!err && userData) {
                      ...
                      if (position > -1) {
                        ...
                        const updatedUserData = { ...userData, checks };
                        lib.update(
                          "users",
                          userEmail,
                          updatedUserData,
                  ...
                  callback(400, {
                    Error: "Deleting check was not successfull.",
                  });
                }
              });
            } else {
